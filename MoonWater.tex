...
\hypersetup{
  ...
  pdftitle={The Moon and Water -- Secondary Circumterrestrial Disk and Non-Collisional Passage of a Massive Planetary Body},
  ...
  pdfkeywords={Moon formation, lunar water, volatiles, D/H ratio, circumterrestrial disk, secondary accretion, non-collisional encounter, orbital sorting, angular momentum, lunar geochemistry, primordial orbital instability, eliminated planet}
}
...
\centering
\vspace*{0.8cm}
{\Huge\bfseries The Moon and Water\par}
\vspace{0.5cm}
{\LARGE Secondary Circumterrestrial Disk\\[3pt]
and Non-Collisional Passage of a Massive Planetary Body\par}
\vspace{0.45cm}
{\Large A physical path from the proto-terrestrial system\\ to the Earth--Moon system\par}
\vspace{0.35cm}
{\normalsize Reservoirs, fluxes, non-collisional planetary encounter,\\
geochemistry, water and viability domain\par}
\vspace{1.1cm}
...
\vspace{0.15cm}
Geophysicist --- M.Sc., Université libre de Bruxelles (ULB)\\
...
$B$ does not supply the bulk of the lunar material: it is a
...
\end{falsif}
\begin{apport}
Highlights what the proposed path contributes in its own right.
\end{apport}
...
The proposed theory solves an inverse problem. The final state is known and non-negotiable: the
Moon exists, with its mass, its iron deficit, its isotopic composition and the angular momentum of
the pair it forms with the Earth. The work therefore consists in working back to the conditions
capable of leading to it, and in determining them at three instants: the state of the proto-Earth
and of the secondary disk before the encounter, the configuration of the passage of planet $B$
during it, and the state of the system immediately afterwards, which serves as the starting point
for the subsequent tidal evolution. These three states are the unknowns of the problem, and the
viable domain is the set of their mutually compatible values.
\label{ch:probleme}
% ---------------------------------------------------------------------
...
In this framework, a planetary body $B$ does not supply the bulk of the lunar material. Its passage remains non-collisional and acts as a transient gravitational agent that
...
and angular-momentum fluxes between the reservoirs of the proto-terrestrial system}. It does not
supply the bulk of the lunar material. Its action begins and ends with its passage.
...
\section{Status of the elements of the path}

The proposed path combines elements whose epistemic status differs, and it is useful to distinguish
them at the outset: one is borrowed from an already established framework, the other is the theory's
own and constitutes what it asks to be examined.
...
The \emph{grazing non-collisional passage of a planet $B$} is the theory's own element. The disk
exists before the encounter; $B$ therefore does not have to create it, and its action is limited to
acting on it through its gravitational field. This economy of means is the distinctive trait of the
proposed path: the matter is already there, the angular momentum is already there, and the external
agent merely redistributes both.

\begin{ideephys}[The principle of precise circumstances]
The Moon exists, and it has the particular properties observation ascribes to it: an unusual mass
for a satellite of a terrestrial planet, an iron deficit, an isotopic composition indistinguishable
from that of the terrestrial mantle, a high angular momentum. These properties do not follow from a
general law; they call for \emph{precise circumstances}. The proposed work therefore consists less
in recounting what happened than in delimiting, in parameter space, the region where these
circumstances are simultaneously realizable. That region is the \emph{viable domain}
$\mathcal{D}_{\mathrm{viable}}$ defined in Chapter~\ref{ch:portes}. If it is non-empty, the
proposed path is physically practicable; if it is empty, it is not. It is in this sense that the
theory is a path theory, and in this sense too that it is falsifiable.
\end{ideephys}

\begin{lecture}
In the proposed path, a gravitational interaction without contact suffices to reorganize an
already constituted reservoir. $B$ retains its physical identity and passes by. Its passage
introduces an external torque able to redistribute the angular momentum of the preexisting
reservoir.
\end{lecture}
...
\centering
\small
\caption{Contrasts between the circumplanetary disks of the giants and the proposed
circumterrestrial reservoir. The differences are not anomalies to be corrected: they define the
problem the theory seeks to solve.}
\label{tab:contraste-disques}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{4.2cm}
>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}X}
\toprule
 & \textbf{Jupiter, Saturn} & \textbf{Earth (proposed)} \\
\midrule
Disk mass & $\sim10^{-4}\,M_p$ & $\sim10^{-2}\,M_p$, to be explained \\[3pt]
Dominant phase & gas and solids & condensates, silicate vapour \\[3pt]
Pressure support & yes, provided by gas & marginal \\[3pt]
Lifetime & millions of years & a few years, Eq.~\eqref{eq:tnu-reservoir} \\
\bottomrule
\end{tabularx}
\end{table}


\paragraph{Why the reservoir is condensate-dominated.} Two independent reasons, worth
distinguishing.

The first is chronological. Protoplanetary disks live three to five million years, and tungsten
chronology places lunar formation more than fifty million years after the CAIs. A factor of ten to
twenty separates the two: nebular hydrogen and helium have long since gone, and no light gas is
available. This point is not a hypothesis; it is fixed by the dating.

The second is energetic. The latent heat of vaporization of silicate is about
$1.2\times10^{7}$~J\,kg$^{-1}$, whereas the thermal energy of a magma at $3000$~K represents only a
tenth of it. Isolated matter can therefore vaporize only a fraction of itself, of order $10$ to
$20\,\%$, vaporization being endothermic and cooling itself as it proceeds. Vaporizing the whole
would require depositing an energy comparable to the binding energy, that is
$3\times10^{7}$~J\,kg$^{-1}$ at $1.7\,\Rt$. Yet the passage deposits none: matter is lifted exactly
at the limit where it ceases to be bound. \emph{The nature of the reservoir therefore follows
directly from the non-collisional character of the mechanism}, and is a consequence of the theory
rather than an added hypothesis.

\paragraph{But the reservoir is not in vacuum.} It is bathed in the radiation of a magma ocean. An
element at distance $r$ from a surface at temperature $T_s$ reaches radiative equilibrium at
$T_d\simeq T_s\sqrt{\Req/r}/\sqrt2$. For $\Req=1.7\,\Rt$, the disk at the Roche limit sits at
$0.54\,T_s$. Since silicate condenses between $1700$ and $2100$~K, a simple criterion follows: the
reservoir remains vapour out to $\aR$ if the surface exceeds about $3700$~K, and condenses into
droplets below that. This threshold is the quantitative form taken by the gate
$\mathcal{D}_{\mathrm{thermo}}$.

The proposed reservoir is therefore a two-phase medium: condensate-dominated by mass, for the
energetic reasons above, but with a vapour component that controls pressure and viscosity and whose
abundance depends directly on the surface temperature. The word \emph{condensates} is preferred to
\emph{solids} deliberately: what leaves the magma ocean is liquid, and solidifies only afterwards by
radiation once in orbit.

\begin{ideephys}[What orbital sorting comes to replace]
In a circumjovian disk it is the gas that supplies pressure support, viscosity and duration: it
leaves the system millions of years to transport angular momentum slowly outward and form
satellites. The proposed circumterrestrial reservoir has none of this. It is solid-dominated,
without pressure support, and its lifetime is counted in years. It therefore needs a mechanism that
accomplishes at once what the gas accomplishes slowly. This is the function we propose to attribute
to the passage of $B$: orbital sorting is the impulsive equivalent of viscous transport. The four
contrasts of Table~\ref{tab:contraste-disques} are therefore not weaknesses of the analogy; they are
the reason for the proposed mechanism.
\end{ideephys}
...
proto-Earth with present terrestrial mass and radius at 1~AU; the values associated with $B$ are
\emph{scale values}, established for $M_B=0.1\,\Mt$, $q_B=2\,\Rt$ and a parabolic trajectory. They
serve to fix orders of magnitude and must not be confused with the reference configuration of the
theory, defined in Section~\ref{sec:equation-lune} by $\tilde\omega=0.97$, $\Req=1.70\,\Rt$,
$\mu_B=0.95$ and $q_B=3.0\,\Rt$.
...
II --- Encounter & $\mathcal{T}_{B\to D}$, $\mathcal{T}_{B\to\oplus}$, $\Phi^{\mathrm{mar}}_{\oplus\to D}$ & $t_{p}\sim$ 0.5--2 h \\
...
\end{equation}
where $t_{p}$ denotes the periapsis scale $q_B/v_p$, about $26$~min for the scale values
of Table~\ref{tab:echelles} and about $0.95$~h for the reference configuration. This scale measures
the periapsis crossing rather than the duration of the forcing: the time over which the
differential field remains comparable to its maximum reaches $5$ to $12$~h depending on the
distance retained, as detailed in paragraph~\ref{par:duree}. Since
$P_{\mathrm{orb}}(\aR)\approx7$~h, the hierarchy above holds for the periapsis scale, but the
assumption of a reservoir frozen throughout the forcing ceases to be uniformly valid. This is why
the Phase~B calculation integrates the complete hyperbolic trajectory instead of applying an
impulse. Figure~\ref{fig:temps} places these scales on a logarithmic axis.
...
\begin{tikzpicture}[x=1cm,y=1cm]
  ...
  \node[font=\scriptsize,violet!60!black,align=center] at (1.477,1.05) {encounter\\$t_{p}$};
...
\label{eq:tnu-num}
\end{equation}
This value is evaluated at $10\,\Rt$. It is useful to restate it at the radius where the reservoir
actually lies, since $\Omega_K\propto r^{-3/2}$:
\begin{equation}
t_\nu\simeq7\ \text{yr at }2\,\Rt,\qquad t_\nu\simeq13\ \text{yr at }2.9\,\Rt
\qquad(\alpha=10^{-3},\ h=0.1),
\label{eq:tnu-reservoir}
\end{equation}
and to note that for a massive reservoir, whose transport is dominated by gravitational torques,
the effective $\alpha$ rises to $10^{-2}$--$10^{-1}$ and reduces these durations to a few months.
The time window offered to the passage is therefore a constraint in its own right, and
Eq.~\eqref{eq:tnu-reservoir} is the reference for gate~\eqref{eq:tpers}.
A particulate component located beyond the Roche limit aggregates in a few months to about a year
\cite{Ida1997,Kokubo2000}. These reference times are compared with
...
\label{eq:eta}
\end{equation}
The notation now distinguishes two durations that must not be conflated: $t_p=q_B/v_p$ is the
periapsis scale, while $t_{\mathrm{eff}}$ denotes the time over which the differential field remains
comparable to its maximum. The second is five to ten times the first.
For $\eta\ll1$ the response is impulsive, for $\eta\sim1$ it is resonant, and for $\eta\gg1$ it is
adiabatic and the forcing is attenuated. For the scale values of Table~\ref{tab:echelles}, the
periapsis scale is $t_{p}\approx26$~min and $\eta(\aR)\approx0.4$: near periapsis, the
region where the Moon assembles receives an impulsive forcing. Referred to the effective duration
of the forcing, $\eta(\aR)$ exceeds unity and the regime becomes resonant, which justifies complete
integration rather than an impulse. For the proto-Earth itself, the analogous parameter
$\eta_\oplus=\Omega_\oplus t_{p}$ is of order unity when the rotation is close to the
...
\section{The magma ocean and the intertropical band}
\label{sec:bande}

Before any equations, it is worth describing the scene, because the geometry of the phenomenon can
be understood without calculation and it governs everything that follows.

\paragraph{The state of the surface.} The proto-Earth considered here has no solid crust in the
present-day sense. Its differentiation is already complete: metallic iron, nickel, cobalt and the
siderophile elements have sunk toward the centre and form the core. What remains above is a hot,
largely molten silicate mantle whose upper part constitutes a magma ocean. Its surface is not
smooth. It is covered by a thin, broken crust, constantly reformed and constantly foundered, whose
closest known analogue is that of \emph{aa} basaltic flows: a pavement of clinkery blocks carried
by a fluid mass, which fractures as soon as it is stressed. This texture is not a picturesque
detail. It means that the mechanical cohesion of the surface is negligible compared with gravity,
and therefore that the only thing holding the matter in place is the effective weight acting on it.

\paragraph{Why a band, and why that one.} Two effects combine, and both are maximal in the same
place.

The first is rotation. On a rapidly rotating body, centrifugal acceleration opposes gravity in
proportion to the square of the cosine of latitude. It therefore vanishes at the poles and is
maximal at the equator. The effective weight is already lowest in the equatorial belt before $B$
even arrives.

The second is the tide of $B$. The differential field pulls matter outward near the point directly
beneath $B$, and inward beyond an angular distance of $54.7^\circ$ from that point. If $B$ passes
at low inclination, its sub-point sweeps precisely the low latitudes.

The two effects therefore superpose in the same belt, and nowhere else. It is from there, and only
from there, that matter can leave.

\paragraph{The lifting condition.} At the equator, matter leaves the surface when the sum of the
centrifugal component and the radial component of the tide exceeds gravity. Dividing by
$G\Mt/\Req^2$, this condition takes a remarkably simple dimensionless form:
\begin{equation}
\boxed{\;2\,\mu_B\left(\frac{\Req}{q_B}\right)^{3}+\tilde\omega^{2}\;\ge\;1.\;}
\label{eq:soulevement}
\end{equation}
It reads directly. The second term is the share supplied by intrinsic rotation, the first the share
supplied by $B$. Neither suffices alone: a proto-Earth at critical rotation only just reaches
equality without $B$, and $B$ alone lifts nothing on a slow body. Lifting is a joint effect, and
this is why the path under study requires both conditions at once.

\paragraph{The extent of the band.} Making the same comparison at latitude $\lambda$, where the
centrifugal term varies as $\cos^2\lambda$ and the tidal term as $3\cos^2\lambda-1$, the lifted
zone extends to the latitude $\lambda_{\max}$ solution of
\begin{equation}
\mu_B\left(\frac{\Req}{q_B}\right)^{3}\left(3\cos^{2}\lambda-1\right)
\;\ge\;1-\tilde\omega^{2}\cos^{2}\lambda .
\label{eq:bande}
\end{equation}
The result is the same across the whole viable domain, which was not obvious in advance:
...
\begin{ideephys}[An intertropical band]
The zone from which matter leaves extends, throughout the viable domain, between $23^\circ$ and
$26^\circ$ of latitude on either side of the equator. That is, to within a degree, the present
terrestrial intertropical band. The coincidence obviously carries no causal meaning, but it gives
an accurate and immediately intelligible picture: the matter of which the Moon is made does not
come from the whole surface of the proto-Earth, it comes from an equatorial belt about a quarter of
the way to the pole. All the rest of the surface stays in place.
\end{ideephys}

\paragraph{What this implies for composition.} The geochemical consequence follows immediately, and
it is not an additional hypothesis. The lifted matter is that of the top of the magma ocean, hence
silicate from which the metal has already departed. Its poverty in iron, nickel and siderophile
elements is not an ingredient to be added to the model: it is a property of the place it comes
from. The mechanism that fixes the mass and the one that fixes the composition are therefore the
same mechanism, and this is the most economical argument in the whole theory.
...
\section{Tidal impulse: analytical form and order-of-magnitude bound}

The impulse received by a reservoir element is given by the closed form of
Appendix~\ref{app:impulsion}, valid under the three stated hypotheses: rectilinear trajectory,
impulsive forcing, fixed particle. The first-order approximation
$\Delta\mathbf{v}\propto(0,y,-z)$ is reliable only for $r\ll q_B$; it loses all precision once
$r/q_B\gtrsim0.3$ and must not serve as reference in the region feeding the lunar window.

To fix orders of magnitude, the bound
\begin{equation}
\frac{\Delta v}{v_K}\sim\sqrt2\,\frac{\mu_B}{\sqrt{1+\mu_B}}
\left(\frac{r}{q_B}\right)^{3/2}
\label{eq:impulsion}
\end{equation}
is used (Appendix~\ref{app:impulsion}). For $\mu_B=0.1$, the amplitude is about
...
The hyperbolic curvature correction is treated in Phase~B by direct integration on the true conic,
and the deviation from the closed form~\eqref{eq:impulsion-close} is an output of the calculation.
This is the purpose of gate $\mathcal{D}_{\mathrm{imp}}$ defined below.
...
time scale (the solar torque is negligible over $t_{p}$). The geocentric frame of
...
\section{The reservoir as an accretion disk}
\label{sec:disque-accretion}

Before entering the particular structure of $\RD$, it is useful to recall that an accretion disk is
a \emph{generic} structure: matter in orbit around a central body, viscous dissipation converting
orbital energy into heat, and outward angular-momentum transport allowing inward accretion. The
three ingredients are inseparable. This is exactly the structure proposed for $\RD$, and the tools
developed for accretion disks in general \cite{ShakuraSunyaev1973,Pringle1981,FrankKingRaine2002}
therefore apply to it without special transposition.

\paragraph{An analogue for the feeding.} Disk formation by mass transfer through the inner Lagrange
point of a binary system is a known and robust mechanism \cite{FrankKingRaine2002}. The body filling
its Roche lobe yields matter to its companion, and that matter, too rich in angular momentum to fall
directly, circularizes into a disk. We propose to see in the flux $\Phi_{\oplus\to D}$ the analogue
of this mechanism: the proto-Earth, brought near its rotation limit and deformed by the differential
field of $B$, yields surface material that circularizes in turn. The analogy is not a proof, and the
geometries differ; it indicates only that the type of flux invoked is not exotic.

\paragraph{The central problem is angular-momentum transport.} In any accretion disk, the angular
momentum lost by matter falling toward the centre must be compensated by a gain of angular momentum
by matter located farther out. Without this transfer, nothing accretes. This is precisely the
function attributed to the orbital sorting of Chapter~\ref{ch:fenetre}: the torque exerted by $B$
moves angular momentum toward the outside of the reservoir, allowing a fraction of the matter to
cross $\aR$ while another fraction falls back inward. The passage therefore plays, once and
impulsively, the role that viscosity plays slowly and continuously.

\paragraph{An additional transport route to consider.} The effective viscosity
$\nu\sim G^2\Sigma^2/\Omega_K^3$ adopted above assumes transport by gravitational torques. If the
reservoir retains residual gas or an ionized fraction, the magnetorotational instability
\cite{BalbusHawley1991} becomes a highly efficient mechanism of angular-momentum redistribution in
weakly magnetized disks, and could complement or replace this effective viscosity. This route is not
adopted here as a baseline hypothesis, because the degree of ionization of $\RD$ is unconstrained;
we propose to treat it as an alternative branch of the viable domain, to be activated if the thermal
state computed in Phase~A indicates sufficient ionization.
...
A particle coupled to the gas is characterized by its Stokes number$\mathrm{St}=t_{\mathrm{stop}}\Omega_K$. The competition between settling and turbulence fixes its
vertical thickness:
\begin{equation}
...
It is instructive to evaluate this criterion on a reservoir of a few lunar masses. For
$M_{D,0}=5.4\,M_L$ distributed as $\Sigma\propto r^{-1}$ between $\Req$ and $\aR$, the surface
density is $\Sigma\simeq7.6\times10^{8}$~kg\,m$^{-2}$ at the inner edge and
$4.5\times10^{8}$~kg\,m$^{-2}$ at the outer edge, and stability $Q_T^{\mathrm{part}}\ge1$ requires
\begin{equation}
\sigma_r\gtrsim0.06\text{--}0.09\;v_K\qquad(330\text{--}400\ \text{m\,s}^{-1}).
\label{eq:sigmar-reservoir}
\end{equation}
A reservoir of this density is therefore not a quiescent dust disk: either it is hot, fluid or
partially molten, or it is already organized into moonlets. The Phase~A calculation decides between
these two states, and that choice determines the nature of the Phase~B equations, since a fluid
reservoir calls for hydrodynamics and a reservoir of moonlets for an $N$-body treatment. The
lighter the reservoir, the lower the required $\sigma_r$ and the more legitimate the independent
particle regime becomes: one more reason to sweep $M_{D,0}$.
...
\end{equation}
To this global condition is added a local one, comparing the reservoir lifetime with the delay
actually available before the encounter:
\begin{equation}
\boxed{\;
t_{\mathrm{pers}}(r)=
...
\label{eq:tpers}
\end{equation}
The four times are respectively viscous spreading~\eqref{eq:tnu-reservoir}, gravitational
fragmentation associated with $Q_T$, re-accretion onto the proto-Earth and reservoir diffusion. The
condition must hold over the whole dense radial interval, not merely on average.
...
\section{Disk--satellite accretion control}
...
\label{eq:lD}
\end{equation}
The reference disk--satellite accretion relation \cite{Ida1997} is
\begin{equation}
...
\begin{controle}
This control is most useful as an upper bound on $M_{D,0}$, and it guides the sweep. For a
reservoir of $5.4\,M_L$ distributed as $\Sigma\propto r^{-1}$ between $\Req$ and $\aR$ in Keplerian
rotation, one obtains $L_{D,0}\simeq3.0\times10^{34}$~kg\,m$^2$\,s$^{-1}$, hence
$\ell_D\simeq0.89$ and
\begin{equation}
M_{L,\mathrm{Ida}}\simeq0.59\,M_D\simeq3.2\,M_L.
\label{eq:controle-Ida}
\end{equation}
The estimator therefore returns a satellite mass larger than the observed one, indicating that the
massive branch of the reservoir is not the branch to explore first. Read the other way, the same
estimator identifies the useful interval: $M_{D,0}\simeq2$--$3\,M_L$ brings $M_{L,\mathrm{Ida}}$
close to $M_{L,\mathrm{obs}}$. This interval should anchor the grid of Table~\ref{tab:grille}, the
missing mass then being supplied by the injection channel $M_{\oplus\to D}$
(Section~\ref{sec:bilan-injection}) rather than by the reservoir alone.
\end{controle}
...
Earth's mantle \cite{Kruijer2015,Touboul2015}. In the present theory, this identity corresponds to
what is expected, since lunar tungsten comes mainly from terrestrial silicate matter; this
expectation must be checked in the viable domain.

\begin{apport}
Two results follow here from the very structure of the proposed path, with no adjusted parameter.

First the Earth--Moon isotopic similarity. If lunar matter comes mainly from the terrestrial mantle,
through the preexisting reservoir and through the flux $\Phi_{\oplus\to D}$, and if the contribution
of $B$ remains small, then identity in O, Ti, Cr and $^{182}$W is not a coincidence to be explained:
it is the direct consequence of provenance. The theory does not make this identity \emph{possible},
it makes it \emph{expected}.

Second the V, Cr and Mn depletions. These elements are moderately siderophile, and their deficit in
the terrestrial mantle is inherited from core segregation. If the flux $\Phi_{\oplus\to D}$ draws
terrestrial silicate material already depleted, the Moon inherits this signature without any need to
invoke a second lunar segregation. The same lineage of matter therefore carries at once the mass,
the metallic-iron deficit and the moderately siderophile element ratios. It is this economy of
hypotheses that we propose as the main argument in favour of the path studied.
\end{apport}
...
constraint to be satisfied \cite{Canup2023}. The framework of three lineages makes it possible to
...
$q_B$ & $\chi_{\mathrm{tide}}\,d_{R,B}$--$10\,\Rt$ & lower bound: gate~\eqref{eq:DtideB}, i.e. $\approx3.0\,\Rt$ \\
...
$\Req/\Rt$ & $1$--$2$ & dilation; reference configuration at $1.7$ \\
Reservoir & $2$--$5\,\Rt$, $\Sigma\propto r^{-p}$, $p\in[0,3]$ & radial structure \\
$M_{D,0}$ & $1$--$8\,M_L$ & initial reservoir mass; priority $2$--$3\,M_L$ \\
$\Omega_\oplus$ & $\tilde\omega\in[0.8,1]$ & controls $M_{\oplus\to D}$ (Section~\ref{sec:bilan-injection}) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{align}
\mathcal{D}_{\mathrm{disk},0}&=\left\{\Theta_I:\ \text{conditions~\eqref{eq:persistance} and~\eqref{eq:tpers} verified at }t_B\right\},
\label{eq:Ddisque}\\
...
\label{eq:Dimp}\\
\mathcal{D}_{J}^{\mathrm{num}}&=\left\{\Theta_I:\ \frac{|J_{\mathrm{before}}-J_{\mathrm{after}}|}{|J_{\mathrm{before}}|}<\epsilon_J^{\mathrm{num}}\right\},
\label{eq:DJ}\\
...
\paragraph{Overview.} The gates are numerous, and they have been introduced chapter by chapter,
each where the physics made it necessary. It is useful to bring them together.
Table~\ref{tab:portes} summarizes them, with what each requires and how far its verification has
progressed. The right-hand column is the most instructive: it shows that the theory is not equally
advanced everywhere, and where the work remains to be done.

\begin{table}[htbp]
\centering
\small
\caption{Summary of the gates of the viable domain. A gate passed is a constraint satisfied; a gate
closed excludes the corresponding branch. The status column indicates whether the verification has
been carried out, sketched, or remains entirely to be done.}
\label{tab:portes}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}p{2.6cm}
>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{2.6cm}}
\toprule
\textbf{Gate} & \textbf{What it requires} & \textbf{Status} \\
\midrule
$\mathcal{D}_{\mathrm{disk},0}$ & the reservoir exists and survives to the passage & to quantify \\
$\mathcal{D}_{\mathrm{thermo}}$ & the thermal state of the reservoir is consistent & to quantify \\
$\mathcal{D}_{\mathrm{grav}}$ & the reservoir remains gravitationally admissible & sketched \\
\midrule
$\mathcal{D}_{\mathrm{nc}}$ & $B$ does not touch the proto-Earth & passed \\
$\mathcal{D}_{\mathrm{tide},B}$ & $B$ is not tidally disrupted & passed \\
$\mathcal{D}_{\mathrm{nt}}$ & the passage remains non-transitory in the required sense & sketched \\
$\mathcal{D}_{B}$ & the mass and trajectory of $B$ are plausible & sketched \\
$\mathcal{D}_{B,\mathrm{hel}}$ & the heliocentric fate of $B$ is admissible & sketched \\
\midrule
$\mathcal{D}_{\mathrm{fen}}$ & a useful fraction reaches the lunar window & passed \\
$\mathcal{D}_{\mathrm{imp}}$ & the closed form of the impulse remains valid & to quantify \\
$\mathcal{D}_{J}^{\mathrm{num}}$ & the integrator conserves angular momentum & to quantify \\
$\mathcal{D}_{J}^{\mathrm{EM}}$ & the final state reaches $L_{\mathrm{EM}}$ & route open \\
$\mathcal{D}_{\mathrm{evac}}$ & $\phi_{\oplus\to B}\ge0.20$ departs with $B$ & to quantify \\
\midrule
$\mathcal{D}_{\mathrm{accr}}$ & the computed lunar mass is the right one & passed \\
$\mathcal{D}_{\mathrm{arch}}$ & a dominant satellite emerges & sketched \\
$\mathcal{D}_{\mathrm{geo}}$ & iron, metal and siderophiles at observed levels & sketched \\
$\mathcal{D}_{\mathrm{iso}}$ & O, Ti, Cr, Si and W indistinguishable from the mantle & sketched \\
$\mathcal{D}_{\mathrm{chrono}}$ & the order of events and the age are respected & sketched \\
\bottomrule
\end{tabularx}
\end{table}

Three gates are today passed on quantitative grounds: non-collision, the integrity of $B$, and the
accretion of one lunar mass. Four require a calculation not yet carried out. The others rest on
established but unquantified arguments. This is an honest map of the state of progress, and it is
worth more than a blanket assertion of consistency.

\paragraph{Gate $\mathcal{D}_{\mathrm{imp}}$.}
This gate checks that the rectilinear approximation used for the closed form of the impulse remains
acceptable within the viable domain. It is passed when the deviation between
form~\eqref{eq:impulsion-close} and direct integration on the hyperbolic conic remains below a fixed
threshold, for example $10\,\%$ on the fraction $\eta_L$. This threshold is a modelling choice, to
be justified according to the precision sought on the injection budget. If the gate is not passed,
the corresponding branch of the viable domain is excluded, and the calculation must be repeated
with the exact hyperbolic closed form (direct numerical integration).

Three clarifications on these gates. The first separates two requirements previously merged:
$\mathcal{D}_J^{\mathrm{num}}$ checks that the integrator conserves total angular momentum, which
validates the calculation; $\mathcal{D}_J^{\mathrm{EM}}$ checks that the state left by the passage
reaches $L_{\mathrm{EM}}$ at the end of the evolution. For the second to be falsifiable,
$J_{\mathrm{loss}}$ must be an \emph{output} of Phase~C, produced by the integrated removal
mechanism, and not a parameter adjusted a posteriori.

The second concerns the integrity of $B$. Both conditions must be evaluated with the parameters of
the reference configuration. For $\mu_B=0.95$, the proper radius of $B$ is
$R_{B,\mathrm{eff}}\simeq0.99\,\Rt$, so that the non-contact condition reads
$q_B>\Req+R_{B,\mathrm{eff}}\simeq2.69\,\Rt$, while tidal disruption imposes
$d_{R,B}\simeq2.75\,\Rt$ from Eq.~\eqref{eq:RocheB}. The two thresholds are therefore now close, and
the second remains the more restrictive.

The nominal periapsis $q_B=3.0\,\Rt$ satisfies both, with a margin of $12\,\%$ on contact and
$9\,\%$ on disruption, that is $\chi_{\mathrm{tide}}\simeq1.09$. The margin adopted in the viable
domain is $\chi_{\mathrm{tide}}=1.05$, which the nominal configuration clears comfortably; a
stricter requirement, $\chi_{\mathrm{tide}}=1.2$, would impose $q_B>3.30\,\Rt$ and place the
nominal configuration outside the domain. The lower bound of Table~\ref{tab:grille} must therefore
be chosen consistently with the value retained for $\chi_{\mathrm{tide}}$.

The third concerns $\mathcal{D}_{\mathrm{accr}}$: the mass to be compared is $M_{L,\mathrm{calc}}$
in the sense of Eq.~\eqref{eq:MLcalc-eff}, that is the sum of the sorting term and the injection
term, and not the product $\eta_L M_{D,0}$ alone.
...
\paragraph{Cross-validation of the impulse.}
Phase~B computes $\Delta\mathbf{v}$ by direct integration of the differential
acceleration~\eqref{eq:adiff} on the true hyperbolic conic of $B$. The result is compared with the
rectilinear closed form of Appendix~\ref{app:impulsion}, and the deviation is quantified as a
function of $e_B$ and $r/q_B$. This deviation is an \emph{output} of Phase~B, not an adjusted
parameter. It feeds gate $\mathcal{D}_{\mathrm{imp}}$.
...
\section{Injection budget: evaluating \texorpdfstring{$M_{\oplus\to D}$}{M_Earth-to-D}}
\label{sec:bilan-injection}

The injection channel introduced in Chapter~\ref{ch:injection} brings to the disk terrestrial
surface material already depleted in metallic iron, nickel, cobalt and siderophile elements. It is
the only term of balance~\eqref{eq:MLcalc-eff} that is a \emph{source}: it does not draw on
$M_{D,0}$ and is therefore not bounded by $\eta_L$. It carries both the mass and the geochemical
signature, which makes it the most rewarding quantity to evaluate. The following protocol computes
it in four steps.

\paragraph{Step 1 --- departure criterion.} A surface element at colatitude $\theta$ leaves the
proto-Earth when the sum of the centrifugal acceleration associated with $\Omega_\oplus$ and the
radial component of the differential field~\eqref{eq:adiff} exceeds local gravity. Threshold~\eqref{eq:seuil} gives this condition at the sub-$B$ point; it must be evaluated over the
whole surface and at every instant, since the active zone moves during the passage.

\paragraph{Step 2 --- angular momentum carried away.} Each element leaves with
\begin{equation}
j_0=\Omega_\oplus\,\Req^2\sin^2\theta+\Delta j_B,
\label{eq:j0-injection}
\end{equation}
where $\Delta j_B$ is the tangential contribution of $B$. The window
criterion~\eqref{eq:fenetre} then applies unchanged: $r_{\mathrm{circ}}=j_0^2/G\Mt\ge\aR$. This step
is decisive, since it decides whether the stripped matter reaches the accretion region or falls
back. The ratio to be bridged is
\begin{equation}
\frac{j(\aR)}{j(\Req)}=\sqrt{\frac{\aR}{\Req}}\simeq1.55
\qquad(\Req=1.20\,\Rt,\ \aR=2.9\,\Rt),
\label{eq:saut-j}
\end{equation}
whereas the tidal impulse~\eqref{eq:impulsion} supplies only about $0.04\,v_K$ at $r=\Req$ for
$\mu_B=0.1$ and $q_B=3\,\Rt$. The intrinsic rotation must therefore supply most of $j_0$, with $B$
crossing the last threshold: this is why $\tilde\omega$ is the dominant parameter of this budget,
and why $M_{\oplus\to D}$ is highly sensitive to it.

\paragraph{Step 3 --- integration.} The surface is tiled in $(\theta,\varphi)$ and in depth, each
element receiving its labels $(\lambda_\Omega,z)$. Elements crossing the threshold are integrated
in the combined field of the oblate proto-Earth and of $B$, along the hyperbolic trajectory, until
the end of the encounter.

\paragraph{Step 4 --- outputs.} The calculation produces $M_{\oplus\to D}$ and its partition between
the fraction beyond Roche, $M_{\oplus\to L}$, and the Roche-interior fraction, which feeds
$\eta_{\mathrm{in}}$; the distribution in $r_{\mathrm{circ}}$; the angular momentum carried away, to
be entered in balance~\eqref{eq:fermeture}; and the distribution in depth and latitude, which feeds
directly the W/V ratio of Eq.~\eqref{eq:WV}.

\paragraph{Where the injected matter lands.} A framing remark, which changes how the expected
results are to be read. The circularization radius of an element leaving the surface in corotation
is, to leading order,
\begin{equation}
r_{\mathrm{circ}}\simeq\tilde\omega^2\,\Req,
\label{eq:rcirc-corot}
\end{equation}
since its specific angular momentum is $\Omega_\oplus\Req^2$ at the equator. Even at the rotation
limit, $\tilde\omega=1$, the injected matter therefore circularizes near $\Req$, that is
$1.2$ to $1.3\,\Rt$, and not beyond $\aR$. The tidal impulse supplied by $B$ at $r=\Req$, which
remains of order a few hundredths of $v_K$ according to Eq.~\eqref{eq:impulsion-close}, does not
bridge the gap.

We propose not to read this result as a loss. The injected matter is neither ejected nor
re-accreted: it joins the Roche-interior disk, that is the fraction $\eta_{\mathrm{in}}$ of
partition~\eqref{eq:partition}, where it is available for the angular-momentum transport described
in Section~\ref{sec:disque-accretion}. The complete chain then becomes: $B$ draws terrestrial
surface material already depleted in metal, this material circularizes near $1.3\,\Rt$, it feeds the
inner reservoir, and viscous or gravitational transport subsequently carries it beyond $\aR$ where
it accretes. The material lineage is the same from one end of the chain to the other, and the
geochemical predictions of Chapter~\ref{ch:traceurs} are therefore unchanged: it is the same
depleted matter that reaches the Moon, whatever route it takes to cross $\aR$.

The consequence is methodological. For the injection channel, $\eta_L$ is not the right efficiency
measure, since it counts \emph{direct} placement in the lunar window. The relevant efficiency is
that of spreading, that is the fraction of $\eta_{\mathrm{in}}$ that transport carries beyond $\aR$
before the reservoir dissipates. Balance~\eqref{eq:MLcalc-eff} must therefore be read with, for the
term $M_{\oplus\to L}$, a spreading efficiency rather than a sorting efficiency.

\begin{controle}
The sweep involves three parameters only, $\tilde\omega$, $q_B$ and $\mu_B$, the others being fixed
by geometry. The expected sensitivity to $\tilde\omega$ is strong, and Eq.~\eqref{eq:rcirc-corot}
indicates that the useful result of this calculation is not the fraction directly reaching $\aR$,
but the total injected mass, its distribution in $r_{\mathrm{circ}}$ and the width of the rotation
window that allows it, to be set against the corresponding spin in gate~\eqref{eq:DJEM}.
\end{controle}

\begin{falsif}
If, over the whole grid, the injected mass remains negligible compared with $M_L$, or if the
spreading time needed to carry $\eta_{\mathrm{in}}$ beyond $\aR$ exceeds the persistence
duration~\eqref{eq:tpers}, then the injection channel cannot compensate for a light reservoir, and
only the massive branch of the reservoir remains open, with the associated angular-momentum
constraints.
\end{falsif}


\section{The equation of the Moon}
\label{sec:equation-lune}

The results of the preceding sections gather into a single expression. The reduction rests on a
simplification that was not visible at the outset.

\paragraph{Both terms have the same form.} A layer leaving from equatorial radius $\varpi$ carries
$j=\Omega_\oplus\varpi^2+\varpi\,\Delta v_B$. The second term, once expanded with the closed form
of the impulse, equals $\varpi\sqrt{G\Mt/\varpi}\;K\,(\varpi/q_B)^{3/2}$, where the powers of
$\varpi$ recombine exactly into $\varpi^{2}$. Both contributions are therefore proportional to the
same factor, and
\begin{equation}
...
\label{eq:Lambda}
\end{equation}
with $x=\varpi/\Rt$. The circularization radius is then, exactly,
\begin{equation}
\frac{r_{\mathrm{circ}}}{\Rt}=x^{4}\,\Lambda^{2}.
\label{eq:rcirc-close}
\end{equation}

\paragraph{A single equation.} The outermost layer, at $x=a\equiv\Req/\Rt$, is the one that travels
farthest. Setting $b\equiv q_B/\Rt$, the condition for it to cross the Roche limit reduces to a
single quantity:
\begin{equation}
\boxed{\;
\mathcal{L}
...
\end{equation}
governed by two threshold values:
\begin{equation}
\mathcal{L}\ \ge\ \sqrt{\aR/\Rt}=1.704
\quad\text{(existence)},
\qquad
\mathcal{L}\simeq1.80
\quad\text{(one lunar mass)}.
\label{eq:seuils-lune}
\end{equation}

\paragraph{The sign, and why it decides everything.} The tidal impulse is tangential. It adds to the
rotational angular momentum when $B$ travels in the same sense as the proto-Earth, and subtracts
from it otherwise. The $+$ sign therefore corresponds to a prograde passage, the $-$ sign to a
retrograde one, and the gap between them is considerable. For the reference configuration below, the
rotation term is $1.265$ and the tidal term $0.535$, giving $\mathcal{L}=1.800$ prograde and
$\mathcal{L}=0.730$ retrograde. The latter is far below the existence threshold: during a retrograde passage, \emph{no matter from
the surface injection channel} crosses the Roche limit. The closure is complete for that channel. It
does not extend to the preexisting reservoir, which follows a different mechanism and for which the
pilot calculation shows a residual $4\,\%$ still reaching the window in the retrograde case
(Table~\ref{tab:pilot-etaL}).

\paragraph{The physical origin of this gap.} It lies in temporal coherence. For the reference
configuration, the rotational angular velocity of the proto-Earth is
$5.43\times10^{-4}$~rad\,s$^{-1}$ and that of $B$ at periapsis $4.83\times10^{-4}$, a ratio of
$1.12$. The two are therefore nearly synchronous: an element of the equatorial belt remains
practically beneath $B$ throughout the encounter, and the impulses it receives accumulate. In the
retrograde case the two angular velocities add instead of compensating, the phase sweeps twice as
fast, and the alternating impulses nearly cancel. This is the same coherence mechanism revealed by
the efficiency ratio of $4.6$ between the two controls of the pilot calculation
(Table~\ref{tab:pilot-etaL}), but the number is obtained here.

\begin{ideephys}[Reference configuration]
The values adopted in the remainder of this work are $\tilde\omega=0.97$, $\Req=1.7\,\Rt$,
$\mu_B=0.95$, $q_B=3.0\,\Rt$, prograde passage. They give $\mathcal{L}=1.800$, exactly the
lunar-mass threshold, and $M_{>\mathrm{R}}=1.05\,M_L$. Three of their properties deserve mention.
The dilation required of the body is only $13\,\%$, which is attainable for a fully molten mantle,
where a radius of $2\,\Rt$ would have required $33\,\%$. The geometric margins, defined as the ratio of $q_B$ to the threshold concerned minus unity, are
$12\,\%$ on contact and $9\,\%$ on the disruption of $B$, that is
$\chi_{\mathrm{tide}}\simeq1.09$. And the ratio of angular velocities is
$1.12$, meaning that the configuration places itself in the regime of maximal coherence without
this being an imposed condition.
\end{ideephys}

The first threshold is exact and universal: it is kinematics, and no combination of parameters
circumvents it. The second depends on the distribution of mass in the outer layers, hence on the
internal structure adopted.

\paragraph{Verification.} Along the contour yielding one lunar mass, extracted numerically and
reported in the preceding section, $\mathcal{L}$ remains constant to $0.4\,\%$:
...
\begin{ideephys}[What the equation of the Moon summarizes]
Only three parameters enter $\mathcal{L}$: the reduced rotation of the proto-Earth, its dilation,
and the pair formed by the mass and approach distance of $B$. All the remaining dynamics of the
passage condenses into these three numbers. The equation says that these three quantities do not
count separately but through the value of a single sum, and that the Moon corresponds to a
particular value of that sum. The viable domain is therefore not a volume in parameter space: it is
a level surface, and the whole question becomes that of the value of the level.
\end{ideephys}

\begin{controle}
Two caveats on use. Equation~\eqref{eq:equation-lune} describes only the injection channel; if the
preexisting reservoir contributes through $\eta_L M_{D,0}$, the mass threshold falls below $1.81$.
And the constant $1.81$ inherits the uncertainty on internal structure, worth a factor of nine on
the mass produced. The shape of the level surface is therefore better established than its
position, and determining this constant is the priority objective of the numerical programme.
\end{controle}



\section{Removal of angular momentum by carried-away matter}
\label{sec:evacuation}

The passage leaves the system with more angular momentum than is needed. This section proposes a
removal mechanism that requires no external ingredient, because it is already contained in the
feeding mechanism.

\paragraph{How large the excess is.} It must first be evaluated correctly, and the point requires
some care. Rotational angular momentum is often written $S_\oplus=k\,\Mt\,\Omega_\oplus\Req^{2}$
with $k\simeq0.25$, which would amount to assuming that the whole planet dilates. But the metallic
core, carrying about one third of the mass, remains compact at the centre, where it contributes
almost nothing to the moment of inertia. Treating separately a core of unchanged radius and a
dilated mantle, the effective coefficient falls to $k_{\mathrm{eff}}\simeq0.19$, and the
corresponding spin drops from $2.9$ to $2.2\,L_{\mathrm{EM}}$ for $\Req=2\,\Rt$ and
$\tilde\omega=0.97$. The excess to be removed is therefore a factor of two, not three.

\paragraph{What the flyby does not do.} It would be tempting to attribute the removal to the
gravity assist experienced by $B$. This is not the right channel. A flyby is elastic in the frame
of the proto-Earth: $B$ leaves with the same relative speed, only its direction changes. What is
exchanged is \emph{heliocentric} angular momentum, whose reservoir amounts to some
$7.6\times10^{5}\,L_{\mathrm{EM}}$ and which constitutes a budget distinct from that of intrinsic
rotation. As for the tidal torque exerted on the proto-Earth during the encounter, it exists but
remains small: for $k_2/Q$ between $0.02$ and $0.10$, a single passage carries away only $0.02$ to
$0.12\,L_{\mathrm{EM}}$. Both routes are therefore to be set aside, and it is useful to say so
explicitly.

\paragraph{What carried-away matter does.} Matter that leaves the system for good takes its angular
momentum with it. And since it departs from the equatorial belt, its specific angular momentum is
$\Omega_\oplus\Req^{2}$, that is $1/k_{\mathrm{eff}}\simeq5.4$ times the mean value for the body.
Each unit of lost mass therefore carries five times more rotation than a unit of mass taken at
random within the planet. The leverage is considerable, and it leads to modest masses:

\begin{center}
\begin{tabular}{cc}
\toprule
\textbf{Target residual spin} & \textbf{Mass to be carried away} \\
\midrule
...
\begin{ideephys}[The feeding mechanism is also the removal mechanism]
The mass mobilized by the overflow reaches some thirty lunar masses. It therefore suffices that
about one fifth of this matter leave the system for good for the angular-momentum balance to close.
And this is precisely what Roche-lobe overflow does in a binary: matter crosses the inner Lagrange
point and flows toward the companion. $B$ lies a few Earth radii away and departs on a hyperbolic
trajectory; whatever it captures leaves with it for good. The same passage thus supplies the mass of
the Moon, its chemical signature, and the removal of the excess angular momentum, without any
external mechanism being invoked.
\end{ideephys}
...
\node[gray!70,font=\scriptsize,fill=white,inner sep=1pt] at (-2.35,2.35) {Roche limit};
% proto-Terre aplatie
...
\draw[line width=2.6pt,red!65!black,opacity=0.75] (-1.30,-0.06) -- (1.30,-0.06);
\node[red!60!black,font=\scriptsize,anchor=north,fill=white,inner sep=1pt] at (0,-1.12) {intertropical band};
...
\node[red!65!black,font=\scriptsize,anchor=south,align=center] at (1.6,0.30) {to $B$,\\ no return};
% matiere qui retombe dans le disque interieur
\draw[->,line width=1.1pt,teal!65!black] (-1.35,0.15) arc (170:250:1.9);
\node[teal!55!black,font=\scriptsize,anchor=east,align=right] at (-1.6,0.55) {to the inner\\ disk};
% matiere qui atteint la fenetre lunaire
\draw[->,line width=1.1pt,violet!70!black] (-1.25,0.55) .. controls (-2.4,1.9) .. (-0.4,3.1);
\node[violet!65!black,font=\scriptsize,anchor=south,align=center] at (-0.4,3.2) {to the lunar window};
\node[font=\scriptsize,anchor=north,align=center] at (0,-1.72) {compact metallic core,\\ dilated silicate mantle};
\end{tikzpicture}
\caption{The three destinations of the lifted matter. The equatorial band, in red, is the only region
from which matter can leave. A small fraction reaches the lunar window beyond the Roche limit and
will form the Moon. The larger part falls back into the inner disk, where viscous transport will
take it up again. And a fraction leaves the system with $B$, carrying away the excess angular
momentum. The same passage thus fulfils all three functions.}
\label{fig:evacuation}
\end{figure}

\paragraph{An additional gate.} This channel is formulated as a condition on the carried-away
fraction:
\begin{equation}
...
\label{eq:Devac}
\end{equation}
where $\phi_{\oplus\to B}$ denotes the share of the mass \emph{stripped from the proto-Earth} that leaves the
terrestrial Hill sphere bound to $B$. This quantity must not be confused with $\eta_{D\to B}$ of
Chapter~\ref{ch:fenetre}, which refers the fraction of \emph{disk} captured by $B$ to the
preexisting reservoir $M_{D,0}$. The two populations and the two denominators differ, and the pilot
calculation indeed gives $\eta_{D\to B}\simeq0$ in the prograde case without this constraining
$\phi_{\oplus\to B}$, which bears on entirely different matter. This restrictive definition is deliberate. It would be tempting to
add $\eta_{\mathrm{esc}}$, the matter merely unbound from the proto-Earth, but that matter does not
disappear: it remains on heliocentric orbits near $1$~AU and part of it will re-accrete within a few
million years. Only the matter departing with $B$ is lost for good, since it crosses the Hill sphere
in its company, and the later heliocentric fate of $B$ changes nothing. $\phi_{\oplus\to B}$ is thus an output
of the Phase~B calculation, which already follows each element and knows which ones end up bound to
$B$.

\paragraph{Consequence for the constants of the problem.} A more massive proto-Earth shifts several
reference quantities, and it is better to quantify them than to leave them hanging. For an excess
mass of $10\,\%$, the Roche limit and the Hill radius, both varying as $\Mt^{1/3}$, increase by
$3.3\,\%$. The existence threshold of the equation of the Moon rises from $1.704$ to $1.73$, and the
lunar-mass constant from $1.80$ to $1.83$, a shift of $1.6\,\%$. That is four times less than the
uncertainty already left by the choice of internal profile. The effect is therefore absorbed, but it
must be entered in the balance rather than neglected on principle.

\begin{controle}
This channel imposes an additional constraint, and that is a good thing: it becomes a prediction. If
$8\,M_L$ leave the system, the proto-Earth was about $10\,\%$ more massive than the present Earth.
This value is compatible with a late-accretion scenario, but it must be entered in the mass balance
and checked for chronological consistency. It is moreover without effect on the isotopic
constraints, since the matter carried away by $B$ does not contribute to the Moon.
\end{controle}


\section{The angular-momentum balance is vectorial}
\label{sec:vectoriel}

The balance has so far been carried out on scalar quantities, which suffices as long as the passage
occurs in the equatorial plane of the proto-Earth. As soon as $i_B\ne0$, this simplification ceases
to be legitimate, and it is worth saying why, for this may be where the most interesting result
still to be obtained lies.

\paragraph{What a scalar treatment leaves out.} The torque exerted by $B$ on an oblate proto-Earth
is not aligned with the rotation axis when the passage is inclined. It possesses a perpendicular
component, which tilts that axis. A scalar balance records the change in magnitude and ignores the
tilt. Yet the tilt is precisely what produces an obliquity, and obliquity is an observable.

\paragraph{Two targets.} The Earth--Moon system presents two orientations that no scalar balance
can generate. The terrestrial obliquity is $23.4^\circ$. And the lunar orbit is inclined by
$5.1^\circ$ to the ecliptic, whereas a moon formed in an equatorial disk should be born in the
plane of the equator. This second value is a recognized open problem.

\begin{ideephys}[A prediction still unexploited]
The proposed path has a free parameter from which it currently draws no benefit: the inclination
$i_B$ of the passage. A vectorial treatment of the torque exerted by $B$ would relate this
inclination to the final orientation of the system, and hence to two measured quantities rather
than one. If a single value of $i_B$ accounted at once for the terrestrial obliquity and for the
inclination of the lunar orbit, the theory would gain a constraint that none of its scalar versions
can supply. This is, in our view, the most promising development to undertake once the mass balance
is closed.
\end{ideephys}

\paragraph{What this requires.} The Phase~B formalism already follows each element in three
dimensions; it suffices to accumulate angular momentum in vectorial rather than scalar form, and to
project the final state into the barycentric frame. Prediction P7, which concerns $i_{D,1}$, then
becomes the first term of a pair $(i_{D,1},\varepsilon_\oplus)$ to be compared with the two observed
values. No new physics is required, only a change in the nature of the quantities followed.


\paragraph{Effective duration of the passage.}\label{par:duree} A distinction is needed here, as it has been a source
of confusion. The characteristic time $\sqrt{q_B^{3}/G(\Mt+M_B)}$ is about one hour for the
reference configuration, but this is not the duration of the encounter: it is only the timescale at
periapsis. The duration over which the differential field of $B$ remains comparable to its maximum
value is considerably longer.

\begin{center}
\begin{tabular}{lc}
\toprule
\textbf{Quantity measured} & \textbf{Duration} \\
\midrule
characteristic time $\sqrt{q_B^{3}/G M_{\mathrm{tot}}}$ & $0.95$~h \\
time spent within $2\,q_B$ & $3.3$~h \\
time spent within $3\,q_B$ & $5.8$~h \\
time spent within $5\,q_B$ & $11.2$~h \\
\bottomrule
\end{tabular}
\end{center}

The effective encounter therefore lasts between five and twelve hours, five to ten times the
characteristic time. This matters for the injection budget: the rotation period of the proto-Earth
being $3.2$~h, a one-hour encounter would present only a quarter of the equatorial belt to the field
of $B$, whereas a six-hour encounter presents nearly two full turns. The whole band is swept, and
the most favourable zone several times over. The parameter
$\eta_\oplus=\Omega_\oplus t_{p}$ is then about $1.5$ rather than $1$; one remains in the
regime where the two timescales merge, which justifies the gate $\mathcal{D}_{\mathrm{imp}}$, but
the value must be corrected. Finally, the velocity at infinity has almost no influence on these
durations, the periapsis velocity being dominated by gravitational infall; the only real lever is
$q_B$, as $q_B^{3/2}$.



\section{A cold body in a torrid environment}
\label{sec:eau}

The preceding section established that overflow works both ways. It remains to examine what $B$
yields, and the answer depends entirely on what it carries at its surface.

\paragraph{The thermal contrast.} $B$ arrives from the outer regions of the system, where rocky
bodies become ice-covered. It enters an environment whose centrepiece is a magma ocean. The flux
received at $3\,\Rt$ from a surface at $3700$~K is $3.4\times10^{6}$~W\,m$^{-2}$, about two thousand
five hundred times the solar constant, and brings the exposed face of $B$ to radiative equilibrium
near $2800$~K. The transition is abrupt: within minutes, an icy surface reaches a temperature
exceeding the melting point of silicate.

\paragraph{Heating detaches nothing.} A hasty conclusion must be avoided here. The escape velocity
of $B$ is about $11$~km\,s$^{-1}$, whereas the thermal velocity of water vapour, even at $2800$~K,
reaches only $2$~km\,s$^{-1}$, less than one fifth. Sublimation alone delivers almost nothing
either: in six hours, the energy received allows $6.5\times10^{18}$~kg to sublimate, five thousandths
of a terrestrial ocean. The latent heat of sublimation is too large and the encounter too brief.

\begin{ideephys}[Heating makes the matter deliverable, the tide delivers it]
The role of the thermal flash lies elsewhere, and it is decisive. A solid ice shell does not flow; a
vapour atmosphere does. The scale height of a water-vapour envelope rises from $14$~km at $300$~K to
$133$~km at $2800$~K: the envelope is dilated by a factor of ten. Its top then approaches the Roche
lobe of $B$, and whatever crosses it flows away. The tidal field expels the matter; heating merely
renders it expellable.
\end{ideephys}

\paragraph{The quantities involved.} For $\mu_B=0.95$ and $q_B=3.0\,\Rt$, the Roche lobe of $B$ lies
at $1.12\,\Rt$ while its rocky body measures $0.99\,\Rt$. A volatile envelope of $874$~km therefore
suffices to reach it; it would represent seven per cent of the mass of $B$, a modest proportion for
a body formed beyond the ice line, and the equivalent of several hundred terrestrial oceans.

On the terrestrial side the demand is negligible: one ocean corresponds to $0.025\,\%$ of the mass of
$B$, ten oceans to $0.25\,\%$. The mechanism therefore operates far from any fine tuning, and
delivers well beyond what is needed long before delivering too little. This is a comfortable regime,
and it should be noted as such.

\paragraph{Lunar water belongs to the same lineage.} It would be tempting to conclude that the Moon
is born dry, the disk at the Roche limit being carried toward $2000$~K, far above the condensation
of water. Measurements forbid that conclusion. The pre-eruptive water content of lunar volcanic
glasses is estimated at several hundred parts per million \cite{Saal2008}, a value comparable to that
of the terrestrial upper mantle, and the hydrogen isotopic composition of lunar water has proved
indistinguishable from that of terrestrial water \cite{Saal2013}.

This observation strengthens the theory rather than hindering it, for the reason that already holds
for oxygen, titanium, chromium and tungsten: \emph{it is the same lineage of matter}. The Moon
contains isotopically terrestrial water because it is made of terrestrial mantle. The relative
deficit is explained by volatility in a hot disk, the isotopic kinship by provenance. Hydrogen thus
joins the list of tracers pointing to a common terrestrial origin.

\begin{controle}
Two points remain to be established, and they are constraining. First chronology: water arriving
during the few hours of the encounter would struggle to mix into the mantle before the lunar
material departs; the spread-out delivery described in Section~\ref{sec:trainee} lifts this
difficulty, but the order of events must be made explicit. Second composition: if $B$ supplies part
of the terrestrial water, its deuterium-to-hydrogen ratio becomes a strong constraint, since
terrestrial water matches carbonaceous chondrites rather than comets. The formation region of $B$
would thereby be constrained, which constitutes a test of the present scenario.
\end{controle}

\section{The trail left by \texorpdfstring{$B$}{B}}
\label{sec:trainee}

One consequence of the mechanism deserves mention, not because it explains anything, but because it
necessarily occurs and because it leaves something behind.

\paragraph{Overflow works both ways.} Section~\ref{sec:evacuation} considered the overflow of the
proto-Earth. But at closest approach the two bodies have nearly identical Roche lobes, and nothing
prevents $B$ from overflowing in turn. A body from the cold regions of the system, if it carries a
volatile envelope a few hundred kilometres thick, sees it dilated by the radiation of the magma
ocean: the flux received at $3\,\Rt$ from a surface at $3700$~K is about two thousand five hundred
times the solar constant, and brings the exposed face of $B$ to around $2800$~K. The scale height of
the envelope is then multiplied by ten, and its top reaches the lobe.

It must be stressed that heating alone detaches nothing. The escape velocity of $B$ is about
$11$~km\,s$^{-1}$, whereas the thermal velocity of water vapour, even at $2800$~K, reaches only
$2$~km\,s$^{-1}$. Heating does not release the matter, it makes it releasable: the tidal field
expels it, which is why it leaves through the Lagrange points rather than in all directions.

\paragraph{Two exits, two fates.} An overflowing envelope does not escape through a single place. It
leaves through both Lagrange points of the configuration. Through the inner point, matter flows
toward the proto-Earth and its disk. Through the outer point, it departs rearward and leaves the
system for good, leaving a trail along the trajectory of $B$.

The two flows are concomitant: they have the same cause and one does not occur without the other.
The trail is therefore not a hypothesis added to the mechanism, but an obligatory consequence of how
it works.

\begin{ideephys}[A spread-out delivery rather than an instantaneous one]
The trail is left in the vicinity of the terrestrial orbit, on heliocentric trajectories close to
that of the Earth. It therefore continues to cross the Earth long after the passage, and re-accretes
progressively over durations of order ten to a hundred million years. This is a useful property, for
it separates two timescales that the passage conflated: the encounter lasts a few hours, but the
supply of matter it initiates spreads over tens of millions of years. A natural chronological
objection, that of the time needed to incorporate into the mantle matter arriving in six hours, is
thereby lifted: most of it precisely does not arrive in six hours.
\end{ideephys}

\paragraph{An order of magnitude, and nothing more.} An independent constraint exists on the supply
of matter subsequent to terrestrial core segregation, established from the abundance of highly
siderophile elements in the mantle. It corresponds to a few tenths of a per cent of the terrestrial
mass, of order $3\times10^{22}$~kg. A volatile envelope of $874$~km on $B$ would represent
$4\times10^{23}$~kg. The fraction of the trail that would have to fall back on the Earth to reach
this order of magnitude is therefore a few per cent.

We confine ourselves here to this observation of availability. It is presented as a by-product of the
mechanism, not as an explanation of the constraint.

\begin{controle}
A reservation must accompany this observation. The ratio of late supplies between the Earth and the
Moon is observed to be of order $10^{3}$, whereas the ratio of collisional cross-sections between the
two bodies is only a few tens. A re-accreted trail therefore does not by itself account for this
gap, which constitutes an open problem independent of the present work. The question remains open,
and it would be imprudent to present it otherwise.
\end{controle}

\section{The constant of the equation of the Moon}
\label{sec:constante-lune}

The equation of the preceding section contains two numbers. The first, $1.704$, is exact and
depends on nothing: it merely expresses the distance to be covered in order to reach the Roche
limit. The second, the one corresponding to one lunar mass, depends on how matter is distributed in
the outer layers of the proto-Earth. It is the only place in the whole theory where the internal
structure of the body enters, and it is worth pausing on.

\paragraph{Why the distribution matters.} The matter that crosses the Roche limit is the matter
leaving from farthest from the rotation axis, that is, the most superficial. Its quantity therefore
depends on the thickness of the outermost layer, and that thickness in turn depends on the
compressibility of the material. A weakly compressed body keeps a great deal of matter near its
surface; a strongly condensed body concentrates its mass at the centre and leaves little at the
surface. For an identical passage, the former therefore supplies more than the latter, and a more
energetic passage is needed to compensate.

This degree of condensation is conventionally described by an index: the higher it is, the more the
mass is concentrated at the centre. The constant of the equation of the Moon depends on it as
follows.

\begin{center}
\begin{tabular}{ccl}
\toprule
\textbf{Index} & \textbf{Constant} & \textbf{Corresponding state of the body} \\
\midrule
$0.5$ & $1.77$ & weakly condensed body, close to incompressible \\
$1.0$ & $1.80$ & compressed silicate, reference case \\
$1.5$ & $1.88$ & hot convective silicate mantle \\
$2.0$ & $2.07$ & very hot mantle, extended magma ocean \\
\bottomrule
\end{tabular}
\end{center}

\begin{ideephys}[An uncertainty that collapses]
The table above is, in our view, the best justification for the detour through the equation of the
Moon. On the mass produced, moving from one end of this range of indices to the other changes the
result by a factor of nine: this was the heaviest uncertainty in the whole numerical programme. On
the constant, the same change shifts the result by only six per cent around $1.9$. The
reformulation has not removed the uncertainty; it has confined it to a quantity where it becomes
negligible. We are therefore no longer looking for an unknown constant: we know it to within a few
per cent, and what remains to be done is to choose the index, not to redo the calculation.
\end{ideephys}

\paragraph{The shape of the domain does not move.} A second result deserves mention, as it was not
obvious in advance. Whatever index is adopted, the constant remains constant along the contour, to
within two or three tenths of a per cent, including when the approach distance is varied and not
only the mass of the perturber. In other words, changing the internal structure shifts the level of
the surface, but does not deform it. The most useful property of the result, the one that reduces
three parameters to a single number, therefore survives the change of profile intact.

\paragraph{A simplification that lightens the demonstration.} The calculation of
Section~\ref{sec:bilan-injection} proceeds through two intermediate quantities: the mobilizable
mass and the fraction of it that crosses the Roche limit. But the second is a ratio whose
denominator is precisely the first. When they are multiplied together to obtain the useful mass,
the mobilizable mass cancels, and only the mass beyond the threshold radius remains. The Roche lobe
therefore serves only to verify that it lies well below that radius, which it does throughout the
domain studied, but it no longer enters the result. This remark shortens the chain of reasoning
without altering its conclusions.

\begin{controle}
The restrictive point lies at the upper end of the table. For a proto-Earth whose outer layers
would be very hot and very dilated, hence for an index near $2$, the domain closes at the approach
distance used so far: reaching one lunar mass would require a dilation such that the proto-Earth
and $B$ would come into contact, and the gate $\mathcal{D}_{\mathrm{nc}}$ closes before the mass is
attained. The domain is not lost for all that, but it moves: it reopens for a perturber of almost
terrestrial mass passing somewhat farther out, with a constant of $2.07$ and the same stability as
elsewhere. The upper bound of the plausible range of indices is therefore also a boundary of the
viable domain, and determining on which side the real proto-Earth lies is the most restrictive
question this chapter raises.
\end{controle}

\section{First pilot calculation of orbital sorting}
\label{sec:pilot-etaL}

A Phase~B calculation was performed to verify that the lunar window \eqref{eq:fenetre} can be
reached over a non-punctual domain of parameters. It constitutes a trend diagnostic rather than a
hydrodynamical proof: the reservoir is represented by $504$ equal-mass test particles, initially
circular and coplanar, distributed between $\Req$ and $\aR$ with $\Sigma\propto r^{-1}$. The
parameters are those of the reference configuration (Section~\ref{sec:equation-lune}):
$\Req=1.70\,\Rt$, $M_B=0.95\,\Mt$, $q_B=3.0\,\Rt$ and $v_\infty=2$~km\,s$^{-1}$, with
$\omega_B=90^\circ$. The acceleration used is the exact differential field of
Eq.~\eqref{eq:adiff}, integrated along the complete hyperbolic trajectory.

\begin{table}[htbp]
\centering
\small
\caption{Partition of the reservoir after the passage, as a function of inclination, for the
reference configuration. Test-particle calculation, without disk self-gravity, without mutual
collisions, without pressure and without hydrodynamics. The last row gives the retrograde control.}
...
\midrule
retrograde, $25^\circ$ & $0.040$ & $0.036$ & $0.246$ & $0.425$ & $0.254$ \\
\bottomrule
\end{tabular}
\end{table}
...
First, the reservoir survives the passage. Despite a perturber mass close to that of the
proto-Earth, the escaping fraction remains below $11\,\%$ and two thirds of the reservoir stay bound
in the inner domain, available for the transport described in Section~\ref{sec:disque-accretion}. An
estimate based on the amplitude of impulse~\eqref{eq:impulsion-close} alone would have predicted
complete dispersal; exact integration shows otherwise, the impulse being only partly tangential and
its sign alternating with azimuth.

Second, the efficiency rises from $20$ to $25^\circ$ and then plateaus, which independently matches
the inclination window imposed by the requirement that $B$ clear the disk.

Third, the retrograde control is unambiguous. The ratio of efficiencies reaches $4.6$, and above all
the structure of the partition changes in nature: in the retrograde case $42.5\,\%$ of the reservoir
escapes and $25.4\,\%$ departs with $B$, against $9\,\%$ and a negligible fraction in the prograde
case. The dispersal of the reservoir that might have been feared does occur, but in the retrograde
sense alone. The temporal coherence established in Section~\ref{sec:equation-lune} therefore
protects the reservoir as well, and not only the injection channel.

For the case retained, $q_B=3\,\Rt$ and $i_B=25^\circ$, we have
\begin{equation}
\eta_L=0.185,\qquad \eta_{\mathrm{in}}=0.655,\qquad
\eta_{\mathrm{reaccr}}=0.067,\qquad \eta_{\mathrm{esc}}=0.093 .
\label{eq:pilot-case}
\end{equation}
For a fixed initial mass, the mass directly placed in the window is
\begin{equation}
M_{L,\mathrm{window}}\simeq0.185\,M_{D,0},
\label{eq:MLwindow}
\end{equation}
which means that one lunar mass immediately placed in this window would require
$M_{D,<R}\simeq5.4\,M_L$ in this test calculation. The notation $M_{D,<R}$ recalls that the $504$
particles all lie initially inside the Roche limit: the inferred mass concerns this tested
population, and not the reservoir as a whole, which may extend farther out. Since no particle
satisfies the window criterion before the passage, one has by construction
\begin{equation}
\eta_L^{(0)}=0,\qquad \Delta\eta_L=\eta_L,
\label{eq:deltaetaL}
\end{equation}
which connects the table directly to the success criterion~\eqref{eq:fenetre-succes}.

These figures are sorting diagnostics, not a lunar formation budget. The quantity $\eta_L$ measures
an orbital sorting efficiency; the sorted matter must still survive relaxation and then accrete.
The relevant mass balance is therefore
\begin{equation}
\boxed{\;
M_{L,\mathrm{calc}}=
...
\label{eq:MLcalc-eff}
\end{equation}
where $\delta M_{L,\mathrm{imp}}$ is the systematic uncertainty introduced by the rectilinear
approximation, estimated by comparing the closed form~\eqref{eq:impulsion-close} with direct
integration on the hyperbolic conic (gate~\eqref{eq:Dimp}).

Two lessons follow. First, since $\epsilon_{\mathrm{acc}}\epsilon_{\mathrm{surv}}\le1$, any
refinement of these efficiencies increases the reservoir mass required at fixed lunar mass, and
hence also $L_{D,0}$, to which it is proportional: the gain is paid on gate
$\mathcal{D}_J^{\mathrm{EM}}$~\eqref{eq:DJEM}. Second, and more importantly, the term
$M_{\oplus\to L}$ does not depend on $M_{D,0}$ at all. It is through this term that the theory can
satisfy $\mathcal{D}_{\mathrm{accr}}$ with a modest reservoir, and it is therefore the quantity to
evaluate first (Section~\ref{sec:bilan-injection}).

\paragraph{Associated angular momentum.}
A reservoir of $5.4\,M_L$ so structured carries, in Keplerian rotation,
\begin{equation}
...
\end{equation}
to which the proto-Earth spin adds about $1.3\,L_{\mathrm{EM}}$ for a $3$~h period. Two useful
consequences. First, since $L_{D,0}$ is proportional to $M_{D,0}$, the light branch of the
reservoir lightens the budget accordingly: one more reason to prefer it. Second, the passage does
not have to supply the angular momentum of the final system, which is already available; its
function is indeed to redistribute it. The state left by the passage must nevertheless reach
$L_{\mathrm{EM}}$ at the end of tidal evolution, which requires an explicit removal mechanism in
a removal mechanism. Three routes are conceivable: capture into solar evection resonance or a
Laplace-plane transition \cite{MurrayDermott1999,TremaineTouma2009}, both belonging to the classical
celestial mechanics of the Earth--satellite system and acting in Phase~C; or the direct carrying
away of matter by $B$ at the time of the passage, which belongs to Phase~B and is the subject of
Section~\ref{sec:evacuation}. This is the purpose of gate
$\mathcal{D}_J^{\mathrm{EM}}$~\eqref{eq:DJEM} and of prediction P8.

The retrograde control of Table~\ref{tab:pilot-etaL} isolates the role of passage direction. For $q_B=3\,\Rt$, at identical
...
\end{equation}
that is, an efficiency about $4.6$ times lower than in the prograde case. The partition moreover
changes in nature, as Table~\ref{tab:pilot-etaL} shows: the escaping fraction rises from $9.3$ to
$42.5\,\%$ and the fraction carried away by $B$ from zero to $25.4\,\%$. This result is consistent
...
hydrodynamics, thermal evolution and balance closure still have to be added. Two clarifications
help to situate it. First its validity domain in mass: for $M_{D,0}=5.4\,M_L$, that is
$0.066\,\Mt$, the reservoir stays below one tenth of $M_B$ and its self-gravity remains small
compared with the measured forcing; the light branch, $M_{D,0}\simeq2$--$3\,M_L$, toward which
control~\eqref{eq:controle-Ida} and criterion~\eqref{eq:sigmar-reservoir} both point, would be more
comfortable still. Table~\ref{tab:pilot-etaL} is therefore to be read
as a trend map in $(q_B,i_B)$, at fixed mass. Second its validity domain in mechanism: the $504$
particles represent the preexisting reservoir alone, with no matter injected from the terrestrial
surface; the map measures $\eta_L$, that is the first term of Eq.~\eqref{eq:MLcalc-eff}, and leaves
$M_{\oplus\to D}$ entirely to be evaluated. Its role is to identify
...
\begin{prediction}{P8. Compatible angular momentum after removal}
\sila The angular momentum of the Earth--reservoir system left by the passage, $J_{\oplus D,1}$,
exceeds $L_{\mathrm{EM}}$ the more so as the reservoir is massive and the proto-Earth fast. The
theory therefore predicts that a post-accretional removal is necessary, and that the pair
$(J_{\oplus D,1},i_{D,1})$ produced by the passage lies in the capture domain of a known mechanism.
\verif Closure of balance~\eqref{eq:fermeture} in Phase~B, then tidal integration of
$J_{\oplus D,1}$ down to $L_{\mathrm{EM}}$, with $J_{\mathrm{loss}}$ an output of the calculation
by evection resonance or Laplace-plane transition
\cite{MurrayDermott1999,TremaineTouma2009}. If no pair $(J_{\oplus D,1},i_{D,1})$ of the viable
domain can be captured, gate~\eqref{eq:DJEM} closes on the corresponding branch.
\end{prediction}
...
significant and $\eta_{\mathrm{esc}}$ moderate, in accordance with~\eqref{eq:fenetre-succes}, while
passing gates $\mathcal{D}_J^{\mathrm{EM}}$, $\mathcal{D}_{\mathrm{grav}}$,
$\mathcal{D}_{\mathrm{disk},0}$, $\mathcal{D}_{\mathrm{accr}}$,
$\mathcal{D}_{\mathrm{tide},B}$ and $\mathcal{D}_{\mathrm{imp}}$.
\verif Phase~B viability maps (Chapter~\ref{ch:programme}), crossed with the injection budget of
Section~\ref{sec:bilan-injection} and the angular-momentum balance.
\end{prediction}
...
$B$ does not supply the bulk of the lunar matter. It is a transient gravitational
...
The Phase~B calculation carried out with the reference configuration gives this sorting a first
value. For $i_B=25^\circ$, $18.5\,\%$ of the reservoir directly satisfies the lunar window criterion
and $65.5\,\%$ remains bound in the inner disk, that is $84\,\%$ of the mass retained in the
circumterrestrial domain, against $9.3\,\%$ escaping. The reservoir therefore survives the passage,
even though the mass of the perturber is close to that of the proto-Earth. The retrograde control,
carried out at identical inclination, separates the two regimes sharply: the window fraction falls to
$4.0\,\%$, the escaping fraction rises to $42.5\,\%$ and a quarter of the reservoir departs with $B$.
The temporal coherence of the prograde passage thus protects the reservoir as much as it feeds the
injection.
...
The same passage provides a physical mechanism for the supply of water. Coming from the cold regions, $B$ enters an environment
whose centrepiece is a magma ocean, receives there a flux of order two thousand five hundred solar
constants, and sees its volatile envelope dilated to the point of overflowing its own Roche lobe.
Heating by itself detaches nothing, the escape velocity being five times the thermal velocity of the
vapour; it renders the matter expellable and the tidal field expels it. Two flows result, one toward
the terrestrial system, the other into a trail whose re-accretion spreads over tens of millions of
years. Lunar water, whose isotopic signature is indistinguishable from terrestrial water, belongs for
its part to the terrestrial lineage, on the same footing as oxygen, titanium, chromium and tungsten.
...
exclude it. The transport law by grazing passage is an \emph{analytical form under hypotheses},
whose validation on the hyperbolic conic is an output of Phase~B and not an adjusted parameter. It
is this validation that transforms the static threshold into a closed model.
...
\section{Tidal impulse: analytical form under hypotheses}
\label{app:impulsion}

\paragraph{Status of this result.}
This section establishes an \emph{analytical reference form} for the tidal impulse received by a
reservoir element during the passage of $B$. It is not exact in the strict sense: it rests on three
explicit hypotheses, stated below. Its domain of validity is specified, and Phase~B of the
numerical program is responsible for validating it by direct integration on the true hyperbolic
conic.

\paragraph{Hypotheses.}
\begin{enumerate}
\item \textbf{Rectilinear trajectory.} The trajectory of $B$ is assimilated to a straight line of
velocity $v_p$ and impact parameter $q_B$ throughout the passage.
\item \textbf{Impulsive forcing.} The encounter duration is short compared with the local orbital
period: $\eta(r)=\Omega_K(r)\,t_{p}\ll1$.
\item \textbf{Fixed particle.} The matter element does not move during the passage; its own orbital
motion is neglected compared with the received impulse.
\end{enumerate}

\paragraph{Conventions.}
Geocentric Cartesian frame $(O;x,y,z)$: the $x$-axis is along the velocity of $B$ at periastron,
the $y$-axis is directed from the proto-Earth center toward $B$ at periastron, the $z$-axis
completes the right-handed triad. The nominal trajectory is
$\mathbf{r}_B(t)=(v_p t,\,q_B,\,0)$.

\paragraph{Derivation.}
The exact differential field is given by Eq.~\eqref{eq:adiff}. Integrating the direct and indirect
terms separately over the rectilinear trajectory gives the following closed form, valid for all
$r<q_B$:
\begin{equation}
\boxed{\;
...
\end{equation}
with the reduced coordinates $\eta=y/q_B$ and $\zeta=z/q_B$.

The first-order approximation in $r/q_B$ follows:
\begin{equation}
...
\label{eq:impulsion-first-order}
\end{equation}
This is the expression historically known in the literature as the impulsive approximation.

\paragraph{Comparison and domain of validity.}
Table~\ref{tab:impulsion-validity} compares the two forms for a particle located on the impact
parameter axis ($\zeta=0$) and on the perpendicular axis ($\eta=0$). The relative deviation is
computed with respect to the closed form~\eqref{eq:impulsion-close}.

\begin{table}[htbp]
\centering
\small
\caption{Deviation between the closed form~\eqref{eq:impulsion-close} and the first-order
approximation~\eqref{eq:impulsion-first-order}.}
\label{tab:impulsion-validity}
\begin{tabular}{ccc}
\toprule
$r/q_B$ & Deviation on the impact parameter axis & Deviation on the perpendicular axis \\
\midrule
$0.1$ & $+11\,\%$ & $-1\,\%$ \\
$0.3$ & $+43\,\%$ & $-8\,\%$ \\
$0.5$ & $+100\,\%$ & $-20\,\%$ \\
...
The first-order approximation loses all precision once $r/q_B\gtrsim0.3$. For the reference case of
the manuscript ($q_B=3\,\Rt$, reservoir up to $\sim2.9\,\Rt$), the ratio $r/q_B$ varies from $0.53$
to $0.97$: the closed form~\eqref{eq:impulsion-close} must therefore be used as the reference, and
the first-order approximation reserved for order-of-magnitude checks.

\paragraph{Order-of-magnitude bound.}
In the limit $r\ll q_B$, the closed form recovers the estimate used in the main text:
\begin{equation}
\frac{\Delta v}{v_K}\sim\sqrt2\,\frac{\mu_B}{\sqrt{1+\mu_B}}
\left(\frac{r}{q_B}\right)^{3/2}.
\label{eq:impulsion-bound}
\end{equation}
This bound is useful for fixing orders of magnitude; it does not replace
form~\eqref{eq:impulsion-close}.

\paragraph{Residual limitation.}
The closed form~\eqref{eq:impulsion-close} still assumes a rectilinear trajectory. For the reference
case ($e_B\simeq1.1$), curvature is not negligible and direct integration on the true hyperbolic
conic is required. This integration is performed in Phase~B, and the deviation from the closed form
is an \emph{output} of the calculation, quantified as a function of $e_B$ and $r/q_B$. It is this
cross-validation that authorizes presenting the transport law as \emph{analytical under hypotheses},
and not as exact in the strict sense.
...
$t_{p}$, $\eta(r)$ & encounter duration, regime parameter $\Omega_Kt_{p}$ \\
...
$\eta_{\mathrm{in}}$, $\eta_{\mathrm{reaccr}}$, $\eta_{\mathrm{esc}}$, $\eta_{D\to B}$ & fractions: inner disk, re-accretion, escape, capture by $B$ \\
...
$\mathcal{D}_{\mathrm{imp}}$ & gate validating the impulse on the hyperbolic conic \\
\bottomrule
\end{longtable}
\normalsize
...
\bibitem{MurrayDermott1999} C.~D. Murray and S.~F. Dermott, \emph{Solar System Dynamics},
Cambridge University Press, 1999.

\bibitem{TremaineTouma2009} S. Tremaine, J. Touma and F. Namouni, ``Satellite dynamics on the
Laplace surface'', \emph{The Astronomical Journal}, 137 (2009), 3706--3717.
doi:10.1088/0004-6256/137/3/3706.

\bibitem{Saal2008} A.~E. Saal, E.~H. Hauri, M. Lo Cascio, J.~A. Van Orman, M.~C. Rutherford and
...
\bibitem{Saal2013} A.~E. Saal, E.~H. Hauri, J.~A. Van Orman and M.~J. Rutherford, ``Hydrogen
...
\bibitem{BalbusHawley1991} S.~A. Balbus and J.~F. Hawley, ``A powerful local shear instability in
weakly magnetized disks. I. Linear analysis'', \emph{The Astrophysical Journal}, 376 (1991),
214--222. doi:10.1086/170270.

\bibitem{ShakuraSunyaev1973} N.~I. Shakura and R.~A. Sunyaev, ``Black holes in binary systems.
Observational appearance'', \emph{Astronomy and Astrophysics}, 24 (1973), 337--355.

\bibitem{FrankKingRaine2002} J. Frank, A. King and D. Raine, \emph{Accretion Power in Astrophysics},
