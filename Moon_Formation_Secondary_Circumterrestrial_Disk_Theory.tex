...
Moon forms. A planet $B$, on a grazing and non-collisional passage, does not supply the bulk of
the lunar material: it acts as a \emph{transient gravitational agent redistributing
...
Dominant phase & gas and solids & solids only \\[3pt]
Pressure support & yes, provided by gas & no \\[3pt]
...
$M_{D,0}=6.9\,M_L$ distributed as $\Sigma\propto r^{-1}$ between $1.6$ and $2.9\,\Rt$, the surface
density is $\Sigma\simeq9.5\times10^{8}$~kg\,m$^{-2}$ at the inner edge and
$5.3\times10^{8}$~kg\,m$^{-2}$ at the outer edge, and stability $Q_T^{\mathrm{part}}\ge1$ requires
\begin{equation}
\sigma_r\gtrsim0.07\text{--}0.10\;v_K\qquad(390\text{--}470\ \text{m\,s}^{-1}).
...
reservoir of $6.9\,M_L$ distributed as $\Sigma\propto r^{-1}$ between $1.6$ and $2.9\,\Rt$ in
Keplerian rotation, one obtains $L_{D,0}\simeq3.8\times10^{34}$~kg\,m$^2$\,s$^{-1}$, hence
$\ell_D\simeq0.88$ and
\begin{equation}
M_{L,\mathrm{Ida}}\simeq0.57\,M_D\simeq3.9\,M_L.
...
\cap\mathcal{D}_{J}^{\mathrm{num}}\cap\mathcal{D}_{J}^{\mathrm{EM}}
...
\label{eq:Dfen}\\
\mathcal{D}_{\mathrm{imp}}&=\left\{\Theta_I:\ \text{the deviation between the closed
...
The second concerns the integrity of $B$. The non-contact condition,
$q_B>\Req+R_{B,\mathrm{eff}}\simeq1.7\,\Rt$ for $M_B=0.1\,\Mt$, is far less restrictive than tidal
disruption, $d_{R,B}\simeq2.75\,\Rt$ from Eq.~\eqref{eq:RocheB}. They therefore deserve to remain
separate, and the factor $\chi_{\mathrm{tide}}>1$ sets the safety margin:
$\chi_{\mathrm{tide}}=1.1$ gives $q_B>3.03\,\Rt$ and $\chi_{\mathrm{tide}}=1.2$ gives
$q_B>3.30\,\Rt$. The lower bound of Table~\ref{tab:grille} must be chosen consistently with
$\chi_{\mathrm{tide}}$; the most grazing columns of the pilot map are then set aside, which is the
expected behaviour of a caution margin.
...
A reservoir of $6.9\,M_L$ so structured carries, in Keplerian rotation,
\begin{equation}
L_{D,0}=2\pi\!\int_{R_{\mathrm{in}}}^{R_{\mathrm{out}}}\!\Sigma(r)\sqrt{G\Mt r}\;r\,\dd r
\simeq3.8\times10^{34}\ \text{kg\,m}^2\,\text{s}^{-1}\simeq1.09\,L_{\mathrm{EM}},
...
Phase~C, for instance capture into solar evection resonance or a Laplace-plane transition
\cite{MurrayDermott1999,TremaineTouma2009}, both belonging to the classical celestial mechanics of
the Earth--satellite system. This is the purpose of gate
...
help to situate it. First its validity domain in mass: for $M_{D,0}=6.9\,M_L$, that is
$0.085\,\Mt$, the reservoir reaches $0.85\,M_B$ and its self-gravity becomes of the same order as
the measured forcing; the test-particle approximation is by contrast legitimate in the light
branch, $M_{D,0}\simeq2$--$3\,M_L$, toward which control~\eqref{eq:controle-Ida} and
criterion~\eqref{eq:sigmar-reservoir} both point. Table~\ref{tab:pilot-etaL} is therefore to be read
as a trend map in $(q_B,i_B)$, at fixed mass. Second its validity domain in mechanism: the $1152$
...
period: $\eta(r)=\Omega_K(r)\,t_{\mathrm{enc}}\ll1$.
